function SalesOpportunityCtrl($scope, $http, $location, $routeParams, $resturls, $rootScope) {
    ...
    $scope.LoadSalesOpportunityList = function (pageIndex) {
        ...
        $http.post($resturls["LoadGoGoCustomerList"], { rank_id: 0, name: '', phone: '', sex: 0, type: 3, pageindex: pageIndex - 1, pagesize: pageSize }).success(function (result) {
            if (result.Error == 0) {
                $scope.salesOpportunitys = result.Data;
                $parent.pages = utilities.paging(result.totalcount, pageIndex, pageSize, '#salesopportunity');
            } else {
