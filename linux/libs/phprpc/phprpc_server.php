...
class PHPRPC_Server {
    ...
    function initKeylen() {
        ...
        else if (isset($_SESSION[$this->cid]['keylen'])){
            $this->keylen = $_SESSION[$this->cid]['keylen'];
        }
        else {
            $this->keylen = 128;
        }
    ...
    function initKey() {
        if (isset($_SESSION[$this->cid]['key'])) {
            $this->key = $_SESSION[$this->cid]['key'];
            require_once('xxtea.php');
        }
        else if ($this->encrypt > 0) {
            $this->errno = E_ERROR;
    ...
    function keyExchange() {
        ...
        if ($this->encrypt === true) {
            ...
            if (!isset($_SESSION[$this->cid])) {
                $_SESSION[$this->cid] = array();
            }
            $_SESSION[$this->cid]['x'] = bigint_num2dec($x);
            $_SESSION[$this->cid]['p'] = $encrypt['p'];
            $_SESSION[$this->cid]['keylen'] = $this->keylen;
            ...
            $x = bigint_dec2num($_SESSION[$this->cid]['x']);
            $p = bigint_dec2num($_SESSION[$this->cid]['p']);
            ...
            $_SESSION[$this->cid]['key'] = str_pad($key, 16, "\0", STR_PAD_LEFT);
        }
        $this->sendCallback();
    }
